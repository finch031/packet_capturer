 ...
 * It only works on the HotSpot JVM, and infers the actual memory
 * layout (32 bit vs. 64 bit word size, compressed object pointers vs. uncompressed)
 * from best available indicators.It can reliably detect a 32 bit vs. 64 bit JVM.
 * It can only make an educated guess at whether compressed OOPs are used, though;
 * specifically, it knows what the JVM's default choice of OOP compression would be
 * based on HotSpot version and maximum heap sizes, but if the choice is explicitly
 * overridden with the <tt>-XX:{+|-}UseCompressedOops</tt> command line switch,
 * it can not detect this fact and will report incorrect sizes, as it will presume
 * the default JVM behavior.
 *
